...
def apple_news(request):
    ...
    datas = [[i.find('img').get('data-src'),i.find('a').text.strip(),i.find('a').get('href')] for i in link]
    return render(request,'web_crawler/apple_news.html',locals())

def yahoo_movie(request):
    url = 'https://movies.yahoo.com.tw/chart.html'
    soup = get_soup(url)
    movie = soup.find('div',class_='rank_list table rankstyle1').find_all('div',class_='tr')
    datas=[]
    for tr in movie[1:]:
        data=[]
        for i,td in enumerate(tr.find_all('div',class_='td')):
            if i == 1:
                ...
                movie_img = soup2.find('div',class_='table').find('div',class_='movie_intro_foto')
                movie_detail = soup2.find('div',class_='table').find('div',class_='movie_intro_info_r')
                data.append(movie_img.find('img').get('src'))
                data.append([i.text.strip() for i in movie_detail.find_all('span')])
            elif i == 5:
...
def get_city(request):
    ...
    for i,tr in enumerate(table):
        if i == 0 :
            ...
            content = [i.text.strip() for i in tr.find_all('td')]
            datas.append(content)
    t=render_to_string('web_crawler/ajax/ibon_table.html',{'datas':datas,'titles':titles})
...
def taiwan_railway(request):
    ...
    times = ['00:00','00:30','01:00','01:30','02:00','02:30','03:00','03:30','04:00','04:30','05:00','05:30','06:00','06:30','07:00','07:30','08:00','08:30','09:00','09:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00','17:30','18:00','18:30','19:00','19:30','20:00','20:30','21:00','21:30','22:00','22:30','23:00','23:30','23:59']
    endtimes = times[:-1]
    sta = get_soup(url).find('div',id='cityHot')
    stations = {btn.text.strip():btn.get('title') for btn in sta.find_all('button')}
...
def get_taiwan_railway_data(request):
    ...
    try:
        soup = get_railway_data(api_url,start_station,end_station,date,start_time,end_time)
        trs = soup.find('div',class_='search-trip').find('table').find_all('tr',class_='trip-column')
        titles = ['車種車次 (始發站 → 終點站)','出發時間','抵達時間','行駛時間','經由','全票','孩童票','敬老票','是否有票']
        datas = []
        for tr in trs[1:]:    
            data = []
...
def selenium(request):  
    return render(request,'web_crawler/selenium.html',locals())

def get_selenium(request):
    chrome = get_Chorme()
