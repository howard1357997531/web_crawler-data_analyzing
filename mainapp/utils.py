...
def get_railway_data(api_url,startStation,endStation,rideDate,startTime,endTime):
    ...
    csrf = soup.find(id="queryForm").find('input',{'name':'_csrf'}).get('value')
    form_data = {
        '_csrf': csrf,
...
def get_data(x,y,num1,num2,a1=None):
    resp = get_soup('https://covid-19.nchc.org.tw/api/covid19?CK=covid-19@nchc.org.tw&querydata=4051&limited=TWN')
...
def get_country_data(x,y,num1,num2,c1=None,c2=None,c3=None,c4=None,c5=None):
    resp = get_soup('https://covid-19.nchc.org.tw/api/covid19?CK=covid-19@nchc.org.tw&querydata=4049')
    soup = json.loads(BeautifulSoup(resp.text,'lxml').find('p').text.strip())
...
def get_sick_data(num,d1=None,d2=None,d3=None,d4=None,d5=None,d6=None):
    resp = get_soup('https://covid-19.nchc.org.tw/api/covid19?CK=covid-19@nchc.org.tw&querydata=4004')
    soup = json.loads(BeautifulSoup(resp.text,'lxml').find('p').text.strip())
...
def get_death_data(num,e1=None,e2=None,e3=None,e4=None):
    resp = get_soup('https://covid-19.nchc.org.tw/api/covid19?CK=covid-19@nchc.org.tw&querydata=4002')
    soup = json.loads(BeautifulSoup(resp.text,'lxml').find('p').text.strip())
    datas = [i[num] for i in soup]
    if e1:
        e1.put(datas)
